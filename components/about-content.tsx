...
import { Github,  Mail,  Rss, MessageCircle, User, Settings } from "lucide-react"
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      <main>
        ...
        {/* Admin panel - subtle icon at the bottom */}
        <div className="mt-12 mb-8 flex justify-center">
          {!isAdmin && !showAdminPrompt && (
            <button
              onClick={() => setShowAdminPrompt(true)}
              className="p-2 rounded-full text-zinc-300 dark:text-zinc-700 hover:text-zinc-500 dark:hover:text-zinc-500 transition-colors"
              title="Admin"
            >
              <Settings size={16} />
            </button>
          ...
          {!isAdmin && showAdminPrompt && (
            <form onSubmit={handleAdminLogin} className="flex items-center gap-2">
              <input
                ...
                autoFocus
                className="h-8 px-3 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 text-xs focus:outline-none focus:ring-1 focus:ring-zinc-400 w-32"
              />
              <button
                type="submit"
                disabled={adminLoading || !adminPassword}
                className="h-8 px-3 rounded-lg bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 text-xs font-medium disabled:opacity-30"
              >
                {adminLoading ? "..." : "Go"}
              </button>
              <button
                type="button"
                onClick={() => { setShowAdminPrompt(false); setAdminError("") }}
          ...
          {isAdmin && (
            <div className="flex gap-3">
              <Link
                href="/create"
                className="flex items-center gap-2 px-4 py-2 rounded-lg border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800/50 transition-colors text-sm"
              >
                New Content
              </Link>
              <Link

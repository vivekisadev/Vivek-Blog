...
export function ContentForm({ initialData, onSubmitAction, isEditMode = false }: ContentFormProps) {
  const [type, setType] = useState<'post' | 'note'>(initialData?.type || 'post')
  ...
  useEffect(() => {
    ...
    if (initialData) {
      if (textareaRef.current) textareaRef.current.value = initialData.content || ''
      if (tagsInputRef.current) tagsInputRef.current.value = initialData.tags || ''
      const titleInput = document.getElementById('title') as HTMLInputElement
      if (titleInput && initialData.title) titleInput.value = initialData.title
    } else {
  ...
  const saveDraft = () => {
    if (isEditMode) return // Don't autosave draft in edit mode
    const titleInput = document.getElementById('title') as HTMLInputElement
    const draft = {
      type,
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.success) {
        ...
        if (isEditMode) {
           router.push(type === 'post' ? `/posts/${initialData?.id}` : `/notes`)
        }
      } else {
    ...
    <>
      <header className="mb-10">
        <h1 className="text-3xl sm:text-4xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100 mb-2">
          {isEditMode ? `Edit ${type === 'post' ? 'Post' : 'Note'}` : `New ${type === 'post' ? 'Post' : 'Note'}`}
        </h1>
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          {isEditMode ? 'Update your content in the digital garden.' : 'Publish your latest thoughts to the digital garden.'}
        </p>
      </header>
            ...
            <div className="pt-6 border-t border-zinc-200 dark:border-zinc-800 flex justify-end">
              ...
              >
                {loading ? (
                  ...
                  <span className="flex items-center">
                    {isEditMode ? 'Save Changes' : `Publish ${type === 'post' ? 'Post' : 'Note'}`}
                    {isEditMode ? <Save className="ml-2 w-4 h-4" /> : <Send className="ml-2 w-4 h-4" />}
                  </span>
                )}
              </Button>
            </div>
